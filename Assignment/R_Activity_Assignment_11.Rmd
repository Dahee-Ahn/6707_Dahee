...
```{r}
library(dplyr)
spider_cleaned <- spider.matrix %>% select(-contains("imm"), -c(LinytoID, LinyToID, unk_toID))
spider_cleaned_1 <- spider_cleaned[rowSums(spider_cleaned[, 8:50] != 0) > 0, ]
...
```{r}
dis.matrix.pa <- vegdist(spider1[, 1:43], method="bray")
print(dis.matrix.pa) ################## How do I provide the distance matrix output????????????
```
